...
import { useCallback, useEffect, useMemo, useRef, useState, type ComponentRef, type Dispatch, type PointerEvent as ReactPointerEvent, type SetStateAction } from 'react'
...
export function usePlacementEditor({
  ...
  const openWorldFolder = useCallback(() => {
    fetch(`/api/open-world-folder?slug=${encodeURIComponent(slug)}&target=scene`).catch((error) => {
  ...
  const saveProject = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/scene-project?slug=${encodeURIComponent(slug)}`, {
        method: 'POST',
  ...
  useEffect(() => {
    if (!editing || !sceneProjectReady || sceneProject || !objects.length) return
    const project: WorldSceneProject = {
      version: projectVersion,
    ...
    fetch(`/api/scene-project?slug=${encodeURIComponent(slug)}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(project),
    })
...
export function PlacementEditorScene({ controller, renderMode }: PlacementEditorSceneProps) {
  ...
  useEffect(() => {
    // three's Object3D event map doesn't declare TransformControls' own events
    // ('mouseDown', 'dragging-changed'), so narrow to the emitter surface we use.
    type TransformControlsEmitter = {
    ...
    const controls = transformRef.current as unknown as TransformControlsEmitter | null
    if (!controls) return

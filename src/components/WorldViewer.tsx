...
import { ArrowsClockwise as ArrowsClockwiseIcon, CaretDown as CaretDownIcon, CaretUp as CaretUpIcon } from '@phosphor-icons/react'
...
export interface WorldViewerProps {
  ...
  onAssetStreamChange?: (state: AssetStreamState) => void
}
...
export function WorldViewer({
  ...
  const assetStreamActive = useProgress((state) => state.active)
  const assetStreamProgress = useProgress((state) => state.progress)
  const assetStreamLoaded = useProgress((state) => state.loaded)
  const assetStreamTotal = useProgress((state) => state.total)
  const assetStreamItem = useProgress((state) => state.item)
  ...
  useEffect(() => {
    onAssetStreamChange?.({
      active: assetStreamActive,
      progress: Number.isFinite(assetStreamProgress) ? Math.round(assetStreamProgress) : 0,
      loaded: assetStreamLoaded,
      total: assetStreamTotal,
      item: assetStreamItem,
    })
  }, [
    assetStreamActive,
    assetStreamItem,
...
function SourceImageControls({
  ...
  if (!activeSourceImageUrl) return null
    ...
    <div className={`pointer-events-none fixed bottom-2 right-2 z-30 hidden md:block ${chrome.enter}`}>
      {activeSourceImageUrl ? (
        thumbnailCollapsed ? (
          ...
          <div className="relative h-96 aspect-square overflow-hidden rounded-lg border border-white/15 bg-black/70 shadow-lg ring-1 ring-black/30 backdrop-blur-md">
            <Image

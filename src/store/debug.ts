...
function preferredViewerQuality() {
...
export const useDebugStore = create<DebugStore>()(
  persist(
    (set) => ({
      // Keep the server and first client render deterministic. Device and
      // persisted preferences are applied explicitly after hydration.
      viewerQuality: ViewerQuality.High,
...
let debugHydration: Promise<void> | undefined

export function hydrateDebugStore() {
  if (typeof window === 'undefined') return Promise.resolve()
  if (debugHydration) return debugHydration

  let hasPersistedQuality = false
  try {
    const raw = window.localStorage.getItem('imageworld-debug')
    const persisted = raw ? JSON.parse(raw) as { state?: { viewerQuality?: unknown } } : undefined
    hasPersistedQuality = Object.values(ViewerQuality).includes(
      persisted?.state?.viewerQuality as ViewerQuality,
    )
  } catch {
    hasPersistedQuality = false
  }

  debugHydration = Promise.resolve(useDebugStore.persist.rehydrate()).then(() => {
    if (!hasPersistedQuality) {
      useDebugStore.getState().setViewerQuality(preferredViewerQuality())
    }
  })
  return debugHydration
}

...
- [ ] **扩大可行走范围（下一步重点）**：SHARP 为单视角重建，只有相机可见表面存在几何——实测角色背后 1/3 的方向完全没有阻挡，走两步即进入虚空。这同时也是**画质**问题的根源：被遮挡处没有数据，偏离原始视角后高斯呈条纹状涂抹（过滤高斯已验证无效，见 PROJECT_REPORT §3.9）。
  - 候选：**Matrix-3D**（MIT 许可、5B low-VRAM 约 12 GB 可在本机跑、输出 `.ply` 直接兼容现有渲染器，但慢——A800 上 720p 约 1 小时）
  - **HunyuanWorld-1.0-lite** 需 18 GB 超出本机 16 GB，且许可排除欧盟/英国/韩国并设 100 万 MAU 上限
  - **HY-World 2.0**（2026）输出可直接导入 Isaac，与本项目已有的 USD 导出契合，显存与许可待确认
  - 真正的岔路口是**算力放在哪跑**：走云 GPU（AutoDL/RunPod）则选择面全开
